...
@MainActor
public class PBPController: ObservableObject {
    @Published public var events: [PBPEventDTO]

    // MARK: - Initialization

    public init(events: [PBPEventDTO] = []) {
        self.events = events
    }

    /// Load events from array
    public func load(events: [PBPEventDTO]) {
        self.events = events
    }
    ...
    public func sortChronological() -> [PBPEventDTO] {
        events.sorted { $0.realWorldTime < $1.realWorldTime }
    }

    /// Sort events reverse chronologically (newest first)
    public func sortReverseChronological() -> [PBPEventDTO] {
        events.sorted { $0.realWorldTime > $1.realWorldTime }
    }

    /// Sort events by period and game time
    public func sortByPeriodAndTime(ascending: Bool = true) -> [PBPEventDTO] {
    ...
    public func sortedWithPeriodMarkers(reverse: Bool = false) -> [PBPEventDTO] {
    ...
    public func filter(by eventType: PBPEventTypeEnum) -> [PBPEventDTO] {
        events.filter { $0.typedEventType == eventType }
    }

    /// Filter events by multiple types
    public func filter(byTypes eventTypes: [PBPEventTypeEnum]) -> [PBPEventDTO] {
        events.filter { eventTypes.contains($0.typedEventType) }
    }

    /// Filter events by period
    public func filter(byPeriod period: Int) -> [PBPEventDTO] {
        events.filter { $0.period == period }
    }

    /// Filter events by team ID
    public func filter(byTeamID teamID: String) -> [PBPEventDTO] {
    ...
    /// Get all goal events
    public var goals: [PBPEventDTO] {
        filter(by: .goal)
    }

    /// Get all shot events (excluding goals)
    public var shots: [PBPEventDTO] {
        filter(by: .shot)
    }

    /// Get all penalty events
    public var penalties: [PBPEventDTO] {
        filter(by: .penalty)
    }

    /// Get all goalkeeper change events
    public var goalkeeperChanges: [PBPEventDTO] {
        filter(by: .goalkeeper)
    }

    /// Get all timeout events
    public var timeouts: [PBPEventDTO] {
        filter(by: .timeout)
    }

    /// Get all shootout events
    public var shootouts: [PBPEventDTO] {
        filter(by: .shootout)
    }

    /// Get all period events
    public var periodEvents: [PBPEventDTO] {
        filter(by: .period)
    }
    ...
    public var timeline: [PBPEventDTO] {
        sortChronological()
    }

    /// Get events in reverse chronological order (most recent first)
    public var reverseTimeline: [PBPEventDTO] {
        sortReverseChronological()
    }

    /// Get events grouped by period
    public func groupedByPeriod() -> [Int: [PBPEventDTO]] {
    ...
    public func goalCount(forTeamID teamID: String) -> Int {
        goals.filter { $0.teamID == teamID }.count
    }

    /// Get penalty count for a specific team
    public func penaltyCount(forTeamID teamID: String) -> Int {
        penalties.filter { $0.teamID == teamID }.count
    }

    /// Get shot count for a specific team (including goals)
    public func shotCount(forTeamID teamID: String, includeGoals: Bool = true) -> Int {
    ...
    public var scoringPlays: [PBPEventDTO] {
        goals.sorted { $0.realWorldTime < $1.realWorldTime }
    }

    /// Get power play goals for a team
    public func powerPlayGoals(forTeamID teamID: String) -> [PBPEventDTO] {
    ...
    public func shortHandedGoals(forTeamID teamID: String) -> [PBPEventDTO] {
        goals.filter { event in
            guard event.teamID == teamID,
                  let goalData = event.data?.asGoal else {
                return false
    ...
    /// Get empty net goals
    public var emptyNetGoals: [PBPEventDTO] {
        goals.filter { event in
    ...
    public var gameWinningGoal: PBPEventDTO? {
    ...
    public var mostRecentEvent: PBPEventDTO? {
        events.max(by: { $0.realWorldTime < $1.realWorldTime })
    }

    /// Get the first event
    public var firstEvent: PBPEventDTO? {
...
extension PBPController {
    ...
    public func filtered(by eventType: PBPEventTypeEnum) -> PBPController {
        PBPController(events: filter(by: eventType))
    }

    /// Create a filtered copy for a specific period
    public func filtered(byPeriod period: Int) -> PBPController {
        PBPController(events: filter(byPeriod: period))
    }

    /// Create a filtered copy for a specific team
    public func filtered(byTeamID teamID: String) -> PBPController {
