...
public class ConnectionMetrics: ObservableObject {
    ...
    @Published public var healthStatus: HealthStatus = .disconnected
    @Published public var isConnected: Bool = false
    @Published public var lastDataReceivedAt: Date?
    @Published public var connectionAttempts: Int = 0
    @Published public var reconnectionCount: Int = 0
    @Published public var errorCount: Int = 0

    // MARK: - Analytics Callback

    /// Closure for reporting analytics events. The app layer can plug in PostHog or any other
    /// analytics provider without the network package depending on it.
    ...
    public func recordConnectionSuccess() {
        ...
        if reconnectionCount > 0 {
            ...
            logAnalyticsEvent(event: "live_listener_reconnected", properties: [
                "reconnection_count": reconnectionCount,
                "total_attempts": connectionAttempts
            ])
        } else {
            logAnalyticsEvent(event: "live_listener_connected", properties: [
                "total_attempts": connectionAttempts
            ])
        }
    ...
    public func recordDisconnection() {
        ...
        logAnalyticsEvent(event: "live_listener_disconnected", properties: [
            "reconnection_count": reconnectionCount,
    ...
    public func recordError(_ error: Error) {
        ...
        logAnalyticsEvent(event: "live_listener_error", properties: [
    ...
    private func logAnalyticsEvent(event: String, properties: [String: Any] = [:]) {
        #if !DEBUG
        ...
        onAnalyticsEvent?(event, enrichedProperties)
        #endif
    }

    /// Log periodic statistics snapshot
    @MainActor
    public func logStatistics() {
        logAnalyticsEvent(event: "live_listener_statistics", properties: [
